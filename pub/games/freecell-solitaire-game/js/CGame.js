function CGame(oData){
    ...
    this._init = function(){
        ...
        _aSuitPos = new Array();           
        _aSuitPos[SUIT_HEARTS]={x: 1062, y:147};
        _aSuitPos[SUIT_SPADES]={x: 1062, y:285};
        _aSuitPos[SUIT_DIAMONDS]={x: 1062, y:419};
        _aSuitPos[SUIT_CLUBS]={x: 1062, y:554};
        ...
        _aFreeCellPos[0]={x: 72, y:147};
        _aFreeCellPos[1]={x: 72, y:284};
        _aFreeCellPos[2]={x: 72, y:419};
        _aFreeCellPos[3]={x: 72, y:555};
        
        _oStartDeckPos = {x: CANVAS_WIDTH/2, y: CANVAS_HEIGHT-100};
        
        var iStartCont=0;
        _aCardStartPos = new Array();
        for (var i=0; i<7; i++){
            for (var j=0; j<8; j++){                
                _aCardStartPos[iStartCont]={x: 217+j*100, y:148+i*_iStackOffset};
                iStartCont++;
            }            
        }
        ...
        if(s_bMobile) {
            ...
            if (window.navigator.msPointerEnabled) {
                ...
                s_oCanvas.addEventListener("MSPointerUp", this.releaseCard, false);
            }else{

                s_oCanvas.addEventListener( 'touchmove', this.onTouchMove, false );
                s_oCanvas.addEventListener( 'touchend', this.releaseCard, false );
            }
        }else{
    ...
    this.onTouchMove = function(event) {
        ...
        if(!_bInitGame && _bBlock){
            ...
            for(var i=0; i<_aStackCard.length; i++){
                _aStackCard[i].setPos(_iCurTouchX - _iMouseOffset.x , _iCurTouchY - _iMouseOffset.y +i*_iStackOffset);
            }
